...
// ============================================================
// PPT (Beamer) LaTeX Generator
// ============================================================
export function generatePPTLatex(pptData) {
...
// ============================================================
// Project Report LaTeX Generator
// ============================================================
...
function normalizeSections(chapter) {
  ...
  // Backward compatibility for older payloads that stored sections as chapter.subsections.
...
export function generateReportLatex(reportData) {
  const { front, chapters, references, annexure } = reportData || {};

  const reportTitle = le((front && front.projectTitle) || 'Project Title Here');
  const reportDept = le((front && front.department) || 'Computer Science & Information Technology');
  const reportYear = le((front && front.year) || '2026');
  const reportSupervisor = le((front && front.supervisor) || 'Prof./Dr./Mr./Ms. xxxx xxxx');

  const authorLines = ((front && front.authors) || [])
